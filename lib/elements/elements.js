...
export async function KarinConvertAdapterMarkdown ({ elements, bot, contact, message_id }) {
  const content = []
  const message = []
  ...
  for (let i of elements) {
    switch (i.type) {
      ...
      case 'reply':
        ...
        break
      case 'image': {
        const { width, height, url } = await bot.uploadMedia({ target_id, target_type, file_data: i.file })
  ...
  if (target_type === 'group') {
    return bot.sdk.sendGroupMessage(target_id, message, bot)
  } else {
    return bot.sdk.sendPrivateMessage(target_id, message)
  }
}

// todo: 后续使用handler代替
/** 拓展api karin转qq-group-bot Templat 使用text_0-9模板 */
export async function KarinConvertAdapterTemplate ({ elements, bot, contact, message_id }) {
  let count = -1
  const params = []
  const message = []
  ...
  // 群
  if (target_type === 'group') {
    return bot.sdk.sendGroupMessage(target_id, message)
  } else {
    return bot.sdk.sendPrivateMessage(target_id, message)
  }
}
