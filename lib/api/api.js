...
import { Bot, KarinMessage, adapter, common, logger, segment, handler } from '#Karin'
...
export default class QQBot extends adapter {
  ...
  /** 日志 加上前缀 */
  logger (level, ...args) {
    logger[level](common.logger(this.account.uid || this.account.uin, ...args))
  }
  ...
  async uploadMedia (file) {
    const res = await handler.call('QQBot.upload.Image', { file })
    if (!res) throw new Error('上传失败，请配置对应的图床插件')
    // 检查返回中是否携带宽高，不携带则计算
    if (res.width && res.height) {
      const { width, height, url } = res
      this.logger('mark', `[uploadMedia][image] ![图片 ${width}px ${height}px](${url})`)
      return res
    }
    file = await common.buffer(file)
    const { width, height } = ImageSize(file)
    this.logger('mark', `[uploadMedia][image] ![图片 ${width}px ${height}px](${res.url})`)
